...
public class Controller2 {
    ...
    */
    ...
    //Multiples ventanas al dar varios click
    //Problemas con las librerías

    public void generarCSV(){
        FileChooser seleccionar = new FileChooser();
        seleccionar.setTitle("Guardar Archivo");
        seleccionar.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("csv","*.csv"));
        File archivo = null;
        ...
        try {
            ...
            archivo = seleccionar.showSaveDialog(null);
            ...
            e.printStackTrace();
            System.out.println("CSV cancelado");
            labelInicio.setTextFill(warningColor);
            labelInicio.setText("CSV cancelado");
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    public void generarExcel(){
        FileChooser seleccionar = new FileChooser();
        seleccionar.setTitle("Guardar Archivo");
        seleccionar.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("Excel","*.xlsx"));
        File archivo = null;
        FileInputStream entrada;
        String texto = "";
        try {
            ...
            if (archivo != null) {
                ...
                FileOutputStream fileOuS = new FileOutputStream(archivo);
                if (archivo.exists()) { // Si el archivo existe lo eliminaremos
                    archivo.delete();
            ...
            e.printStackTrace();
            System.out.println("Excel cancelado");
            labelInicio.setTextFill(warningColor);
            labelInicio.setText("Excel cancelado");
        }catch (FileNotFoundException e) {
            e.printStackTrace();
            System.out.println("Archivo no encontrado");
        } catch (SQLException e) {
